...
# ### Subset dataset to just French data
# french_indices = df[df['Language_type_labels']==1].index
# df = df.loc[french_indices]

### Find length of majority and minority class data
minority_class_len = len(df[df['Demand']>0])
...
onehotlabels_train = enc.transform(x_train[['Auteur_labels','Editeur_labels',
    'Pays_labels','Document_type_labels','Language_type_labels']])
onehotlabels_test = enc.transform(x_test[['Auteur_labels','Editeur_labels',
    'Pays_labels','Document_type_labels','Language_type_labels']])
