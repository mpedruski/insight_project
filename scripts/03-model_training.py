...
def major_classes_undersampler(data, variable, class_to_match):
    ...
    ### map 1 to 1 to demand categories, which is true for my data, but my not be for all
...
def random_accuracy(regr,x_test,y_test):
    y_pred = regr.predict(x_test)
    conf_mat = confusion_matrix(y_test, y_pred)
    total_sum = sum(sum(conf_mat))
...
random_accuracy(regr_1,x_test,y_test)
