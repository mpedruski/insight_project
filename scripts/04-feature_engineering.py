...
if __name__ == "__main__":
    ...
    files_to_write=['expanded_feature_labels_for_each_title.csv', 'expanded_variable_dictionaries.joblib']
    ...
    results = [titles_label_values[0],titles_label_values[1],titles_label_values[2],
        year_column,titles_label_values[3],titles_label_values[4],df['Demanded'],df['Total']]
    df1 = pd.DataFrame(results).transpose()
    df1.columns = ["Auteur_labels","Editeur_labels","Pays_labels","Years_offset",
        "Document_type_labels","Language_type_labels","Demand","Total"]
    ...
    dictionaries = [dict(zip(names_list[0], df.groupby('Auteur')['Lifetime'].sum())),
        ...
        dict(zip(names_list[3], df.groupby('Type-document')['Lifetime'].sum())),
        dict(zip(names_list[4], df.groupby('Langue')['Lifetime'].sum()))]
